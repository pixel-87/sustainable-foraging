...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--cpu", action="store_true", help="Force CPU even if CUDA is available (CleanRL)"
    )
    parser.add_argument("--gamma", type=float, default=0.99, help="Discount factor")

    # MAPPO specifics
    parser.add_argument(
        "--num-steps", type=int, default=128, help="Rollout length per env (CleanRL MAPPO)"
    )
    parser.add_argument("--gae-lambda", type=float, default=0.95, help="GAE lambda (CleanRL MAPPO)")
    parser.add_argument(
        "--num-minibatches", type=int, default=4, help="Number of minibatches (CleanRL MAPPO)"
    )
    parser.add_argument(
        "--update-epochs", type=int, default=4, help="PPO update epochs (CleanRL MAPPO)"
    )
    parser.add_argument(
        "--clip-coef", type=float, default=0.2, help="PPO clip coef (CleanRL MAPPO)"
    )
    parser.add_argument("--ent-coef", type=float, default=0.01, help="Entropy coef (CleanRL MAPPO)")
    parser.add_argument(
        "--vf-coef", type=float, default=0.5, help="Value func coef (CleanRL MAPPO)"
    )
    parser.add_argument(
        "--max-grad-norm", type=float, default=0.5, help="Max grad norm (CleanRL MAPPO)"
    )
    parser.add_argument(
        "--no-anneal-lr",
        action="store_false",
        dest="anneal_lr",
        help="Disable LR annealing (CleanRL MAPPO)",
    )
    parser.set_defaults(anneal_lr=True)

    # DQN specifics
    parser.add_argument(
        "--buffer-size", type=int, default=50_000, help="Replay buffer size (CleanRL DQN)"
    )
    parser.add_argument(
        "--tau", type=float, default=1.0, help="Target network update rate (CleanRL DQN)"
    )
    parser.add_argument(
        "--target-network-frequency",
        type=int,
        default=500,
        help="Steps between target updates (CleanRL DQN)",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="Fraction of total steps for epsilon decay (CleanRL DQN)",
    )
    parser.add_argument(
        "--learning-starts",
        type=int,
        default=1000,
        help="Steps before training begins (CleanRL DQN)",
    )
    parser.add_argument(
        "--train-frequency",
        type=int,
        default=4,
        help="Steps between training updates (CleanRL DQN)",
    )
...
def main() -> None:
    ...
    if args.library == "sb3":
        if args.algorithm not in ["ppo", "a2c"]:
            raise ValueError(
                f"Algorithm {args.algorithm} not supported by sb3 runner (supported: ppo, a2c)."
            )
        ...
        if args.algorithm not in ["mappo", "dqn", "vdn", "qmix"]:
            raise ValueError(
                f"Algorithm {args.algorithm} not supported by cleanrl runner (supported: mappo, dqn, vdn, qmix)."
            )
        ...
        if args.algorithm != "ppo":
            raise ValueError(
                f"Algorithm {args.algorithm} not supported by rllib runner (supported: ppo)."
            )
