...
class BenchmarkCallbacks(DefaultCallbacks):
    ...
    def on_episode_end(self, *, episode: Any, env_index: int, **kwargs: Any) -> None:
        ...
        if not last_step_infos:
            return
        first_agent = next(iter(last_step_infos.keys()))
...
def run_rllib(args: argparse.Namespace) -> None:
    ...
    first_agent = next(iter(dummy_env.get_agent_ids()))
    ...
    while total_steps < args.timesteps:
        ...
        print(
            f"  Steps: {total_steps:>8,}/{args.timesteps:,} | Reward: {result.get('episode_reward_mean', 0.0):>7.2f}"
        )
