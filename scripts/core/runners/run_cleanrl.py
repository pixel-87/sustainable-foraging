...
def _run_mappo(args: argparse.Namespace) -> None:
    ...
    env, _ = make_env(args.preset, num_envs=args.num_envs, vectorize_for_cleanrl_sb3=True)
...
def _run_dqn(args: argparse.Namespace) -> None:
    ...
    env, _ = make_env(args.preset, num_envs=args.num_envs, config_overrides={"grid_observation": False}, vectorize_for_cleanrl_sb3=True)
    obs_dim = int(np.prod(env.observation_space.shape))
    act_dim = int(env.action_space.n)
    num_envs_total = env.num_envs
...
class JointReplayBuffer:
    def __init__(self, capacity: int, num_agents: int, obs_dim: int, device: torch.device):
        self.capacity = capacity
...
def _run_vdn(args: argparse.Namespace) -> None:
    ...
    act_dim = int(env.action_space.n)
    num_envs_total = env.num_envs
    num_agents = num_envs_total // args.num_envs
    ...
    target_network.load_state_dict(q_network.state_dict())
    optimizer = optim.Adam(q_network.parameters(), lr=args.lr)
    rb = JointReplayBuffer(args.buffer_size, num_agents, obs_dim, device)

    global_step = 0
    next_obs_np, _ = env.reset()
    next_obs_flat = next_obs_np.reshape(args.num_envs, num_agents, -1)

    for global_step in range(1, args.timesteps + 1):
        epsilon = linear_schedule(args.start_e, args.end_e, int(args.exploration_fraction * args.timesteps), global_step)

        obs_tensor = torch.tensor(next_obs_flat, dtype=torch.float32, device=device).reshape(-1, obs_dim)
        if random.random() < epsilon:
            actions_flat = np.array([env.action_space.sample() for _ in range(num_envs_total)])
        else:
            with torch.no_grad():
                actions_flat = q_network(obs_tensor).argmax(dim=1).cpu().numpy()

        actions_grouped = actions_flat.reshape(args.num_envs, num_agents)

        new_obs_np, reward_np, terminated_np, truncated_np, infos = env.step(actions_flat)
        done_np = np.logical_or(terminated_np, truncated_np)
        
        new_obs_flat = new_obs_np.reshape(args.num_envs, num_agents, -1)
        reward_grouped = reward_np.reshape(args.num_envs, num_agents)
        done_grouped = done_np.reshape(args.num_envs, num_agents)

        for i in range(args.num_envs):
            # Shared global reward
            global_reward = reward_grouped[i].sum()
            # Joint done if any agent is done
            global_done = done_grouped[i].any()
            
            # Store joint transition
            rb.add(next_obs_flat[i], new_obs_flat[i], actions_grouped[i], np.full(num_agents, global_reward), global_done)
            
            # Extract info metrics (assuming parallelized env duplicates info, just take agent 0's metrics)
            info_idx = i * num_agents
            info_i = infos[info_idx] if isinstance(infos, (list, tuple)) else infos
            if isinstance(infos, dict): info_i = infos
            if "episode_metrics" in info_i:
                metrics = info_i["episode_metrics"]
                tracker.on_episode_end(global_step, metrics)
        ...
        if global_step > args.learning_starts and global_step % args.train_frequency == 0:
            ...
            # VDN Forward pass
            # s_obs shape: (batch_size, num_agents, obs_dim)
            s_obs_flat = s_obs.reshape(-1, obs_dim)
            s_next_obs_flat = s_next_obs.reshape(-1, obs_dim)
            
            with torch.no_grad():
                target_q_flat = target_network(s_next_obs_flat) # (batch*num_agents, act_dim)
                target_max_flat, _ = target_q_flat.max(dim=1)
                target_max = target_max_flat.reshape(args.batch_size, num_agents)
                
                # Q_tot_next = sum(max_Q_i_next)
                q_tot_next = target_max.sum(dim=1)
                
                # Global reward (just take agent 0 since it's shared)
                r_tot = s_rewards[:, 0]
                d_tot = s_dones[:, 0]
                
                td_target = r_tot + args.gamma * q_tot_next * (1 - d_tot)

            current_q_flat = q_network(s_obs_flat)
            s_actions_flat = s_actions.reshape(-1)
            q_i = current_q_flat.gather(1, s_actions_flat.unsqueeze(1)).squeeze()
            q_tot = q_i.reshape(args.batch_size, num_agents).sum(dim=1)
...
def _run_vdn(args: argparse.Namespace) -> None:
    ...
    for global_step in range(1, args.timesteps + 1):
        ...
        env_rewards = reward_np.reshape(args.num_envs, n_agents).sum(axis=1)
        env_dones = done_np.reshape(args.num_envs, n_agents).all(axis=1)
