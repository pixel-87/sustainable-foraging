...
class ForagingEnv(AECEnv):
    ...
    def spawn_food(self, max_num_food, min_levels, max_levels):
        ...
        # permute food levels
        food_permutation = self._np_random.permutation(max_num_food)
    ...
    def spawn_players(self, min_player_levels, max_player_levels):
        ...
        player_permutation = self._np_random.permutation(len(self.players))
        ...
        for player, min_player_level, max_player_level in zip(
            ...
            while attempts < 1000:
                ...
                if self._is_empty_location(row, col):
                    player.setup(
                        (row, col),
                        self._np_random.integers(min_player_level, max_player_level + 1),
                        self.field_size,
                    )
                    break
                attempts += 1
    ...
    def reset(self, seed=None, options=None):
        ...
        player_levels = sorted([player.level for player in self.players if player.level is not None])
